...
from cs336_basics import tokenizer, train_bpe, linear, embedding, rmsnorm, positionwise, rope, softmax, head, multi_head, transformer, train
...
def run_swiglu(
    ...
    swiglu = positionwise.SwiGLU(d_model=d_model, d_ff=d_ff)
...
def run_scaled_dot_product_attention(
    ...
    """
    return head.scaled_dot_product_attention(Q, K, V, mask)
...
def run_multihead_self_attention(
    ...
    """
    model = multi_head.CausalMultiHeadSelfAttention(d_model=d_model, num_heads=num_heads)
    ...
    model.output_proj.weight.data.copy_(o_proj_weight)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    model = multi_head.CausalMultiHeadSelfAttention(d_model=d_model, num_heads=num_heads, theta=theta, max_seq_len=max_seq_len)
    model.q_proj.weight.data.copy_(q_proj_weight)
    model.k_proj.weight.data.copy_(k_proj_weight)
    model.v_proj.weight.data.copy_(v_proj_weight)
    model.output_proj.weight.data.copy_(o_proj_weight)
...
def run_transformer_block(
    ...
    """
    model = transformer.TransFormerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, theta=theta, max_seq_len=max_seq_len)
...
def run_transformer_lm(
    ...
    """
    model = transformer.Transformer(vocab_size=vocab_size, context_length=context_length,
                                    d_model=d_model, num_layers=num_layers, num_heads=num_heads, d_ff=d_ff, rope_theta=rope_theta)
    model.load_state_dict(weights)
...
def run_rmsnorm(
    ...
    """
    model = rmsnorm.RMSNorm(d_model, eps)
    state_dict = {"weight": weights}
    model.load_state_dict(state_dict)
    return model(in_features)
